...
public class AddConeController implements EventHandler<ActionEvent> {
	...
	public void handle(ActionEvent event) {
		...
		addBtn.setOnAction(e -> {
			...
			if (name.getText().isEmpty() || id.getText().isEmpty()) {
				...
				alert.setContentText("Please fill out all of the fields for the new cone");
				alert.showAndWait();
			} else
				try {
					if (id.getText().length() > 4 || id.getText().length() == 4 && checkdatabase()) {
						...
						loader.setLocation(AddConeController.class.getResource("MenuConfigurations.fxml"));

						String query = "insert into cones value \n ('" + id.getText() + "', '" + name.getText()
								+ "', '" + 0 + "');";
	...
	private boolean checkdatabase() throws SQLException {
		String query = "select * from cones";
